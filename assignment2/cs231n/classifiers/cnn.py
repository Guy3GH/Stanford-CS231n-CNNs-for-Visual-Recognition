...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # unpack input dims to get C, H, W
        C, H, W = input_dim

        # conv layer. independent of the original input size, just filter size here!
        self.params['W1'] = np.random.randn(num_filters, C, filter_size, filter_size) * weight_scale        
        self.params['b1'] = np.zeros(num_filters)

        # filtering method of conv = Same. Stride = 1, choosing padding size to keep same dims.

        # maxpool output dims
        HP, WP = (H - 2)//2 + 1, (W - 2)//2 + 1  # 2x2 max pooling, with S = 2
        
        # hidden affine layer params
        self.params['W2'] = np.random.randn(num_filters * HP * WP, hidden_dim) * weight_scale # C from first layer is gone in conv phase.
