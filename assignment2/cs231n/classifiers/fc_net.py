...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grads['W2'] = dW2 + self.reg * W2 # 2*0.5=1
        grads['b2'] = db2
        grads['W1'] = dW1 + self.reg * W1 # 2*0.5=1
        grads['b1'] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers+1):
          ...
          if use_batchnorm and i!=self.num_layers: # without the outputs of the last layer
    ...
    def loss(self, X, y=None):
        ...
        # all layers except softmax (last) layer
        for i in range(1, self.num_layers): 
          ...
          if self.use_batchnorm:
            ...
            current_input, affine_bn_relu_cache[i] = affine_bn_relu_forward(current_input, W, b, gamma, beta, bn_param)   
          else:
            current_input, affine_relu_cache[i] = affine_relu_forward(current_input, W, b)
          if self.use_dropout:
        ...
        #last layer:
        W = self.params['W' + str(self.num_layers)]
        loss += 0.5 * self.reg * (np.sum(W ** 2)) # add regularization loss
        affine_dx, affine_dw, affine_db = affine_backward(dscores, affine_cache)
        grads['W' + str(self.num_layers)] = affine_dw + self.reg * W # 0.5 * 2 = 1
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            # same as with the softmax layer, above:
            W = self.params['W' + str(i)]
            loss += 0.5 * self.reg * (np.sum(W ** 2)) # add regularization loss
            grads['W' + str(i)] = affine_dw + self.reg * W # 0.5 * 2 = 1
