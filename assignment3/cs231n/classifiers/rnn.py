...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (3)
        # select the right forward function
        ...
        # (3)
        # select the right backward function
    ...
    def sample(self, features, max_length=30):
        ...
        c = np.zeros(h.shape)
        V, W = W_embed.shape
        x = np.ones((N, W)) * W_embed[self._start] # (1)

        for t in range(max_length):
            if self.cell_type == "rnn":
               next_h, _ = rnn_step_forward(x, h, Wx, Wh, b) # (2)
            else:
               next_h, next_c, _ = lstm_step_forward(x, h, c, Wx, Wh, b) # (2)
               c = next_c
            h = next_h 
            out = next_h.dot(W_vocab) + b_vocab # (3)
            captions[:, t] = out.argmax(axis=1) # (4). chosen word 
            x = W_embed[captions[:, t]] # next input x
