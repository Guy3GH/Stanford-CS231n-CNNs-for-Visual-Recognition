...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        next_h, cache_h = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
        prev_h = next_h
        h[:, t, :] = prev_h
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_ag = dh[:, t, :] + dprev_h
        dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh_ag, cache[t])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
        dx[:, t, :] = dx_step
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # from now on all multiplications are element-wise
    i = sigmoid(a_i)                                    # (N, H)
    f = sigmoid(a_f)                                    # (N, H)
    o = sigmoid(a_o)                                    # (N, H)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      next_h, c, forward_cache[t] = lstm_step_forward(x[:, t, :], next_h, c, Wx, Wh, b)
      h[:, t, :] = next_h
