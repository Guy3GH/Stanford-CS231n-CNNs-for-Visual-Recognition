...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
      scores = X[i].dot(W) # shape: 1 x C
      scores -= scores.max() # avoiding numeric instability
      probs = np.exp(scores)/np.sum(np.exp(scores)) # softmax
      loss += -np.log(probs[y[i]])

      dscores = probs.reshape(1,-1)
      dscores[:, y[i]] -= 1 # modifying for the correct label index
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    scores -= scores.max(axis = 1, keepdims = True) # avoiding numeric instability
    probs = np.exp(scores)/np.sum(np.exp(scores), axis = 1, keepdims = True) # softmax
    loss = -np.log(probs[np.arange(num_train), y])
    loss = np.sum(loss) # sum over instances

    dscores = probs.reshape(num_train, -1)
    dscores[np.arange(num_train), y] -= 1 # modifying for the correct label index
